...
static
int parse_expected_length(const char *file, const int header_len)
{
    char *len_line = NULL;
    int expected_len = 0;
...
static
bool check_line_length(const int current_len, const int expected_len)
{
...
static
bool process_newline(int *current_len, const int expected_len, int *i,
    const char *file)
{
    if (file[*i] == '\n') {
...
static
bool validate_lines(const char *file, const int start, const int expected_len)
{
    int i;
...
static
bool check_line(const int number_of_line, char *len_line)
{
    if (number_of_line == 0 || number_of_line != (my_getnbr(len_line) + 1)) {
        mini_printf("Error: The number of lines in the file does not "
                    "match.\n");
...
bool check_the_number_of_the_line_in_file(const char *file)
{
    ...
    for (; file[len] != '\0'; len++)
        if (file[len] == '\n')
            number_of_line++;
    if (check_line(number_of_line, len_line) == false)
        return false;
    free(len_line);
    return true;
}
